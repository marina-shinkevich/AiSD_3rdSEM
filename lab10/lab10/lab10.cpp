...
struct GRAPH {
    ...
    GRAPH(int size) : SIZE(size), gr(size, vector<pair<float, float>>(size)) { //Этот конструктор создаёт граф с заданным количеством узлов (size)
        random_device rd;
        mt19937 gen(rd());
